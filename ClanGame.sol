...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Clan Game Dividend Tracker", "Clan Game Dividend Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 500000 * (10**18); //must hold 500,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Clan Game Dividend Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Clan Game Dividend Tracker: WithdrawDividend disabled. Use the 'claim' function on the main Clan Game contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Clan Game Dividend Tracker: ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Clan Game Dividend Tracker: Cannot update claimWait to same value");
...
contract ClanGame is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _busdDividendRewardsFeeBuy;
    uint256 public _marketingFeeBuy;
    uint256 public _LpFeeBuy;
    uint256 public totalBuyFees ;

    uint256 public _busdDividendRewardsFeeSell;
    ...
    constructor() ERC20("Clan Game", "CGAME") {
        _busdDividendRewardsFeeBuy = 5;
        _LpFeeBuy = 1;
        _marketingFeeBuy = 1;
        totalBuyFees = _busdDividendRewardsFeeBuy.add(_marketingFeeBuy).add(_LpFeeBuy);

        _busdDividendRewardsFeeSell = 5;
        _LpFeeSell = 2;
        _marketingFeeSell = 2;  
        totalSellFees = _busdDividendRewardsFeeSell.add(_LpFeeSell).add(_marketingFeeSell);

    	marketingWallet = 0x55963b913394d1F4304024ABa1C9E7DAD5D9b456; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Clan Game: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "Clan Game: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "Clan Game: The new dividend tracker must be owned by the Clan Game token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Clan Game: Account is already exluded from fees");
    ...
    function setTokenRewardsBuyFee(uint256 value) external onlyOwner {
        _busdDividendRewardsFeeBuy = value;
        totalBuyFees = _busdDividendRewardsFeeBuy.add(_marketingFeeBuy).add(_LpFeeBuy);
    }

    function setTokenRewardsSellFee(uint256 value) external onlyOwner {
        _busdDividendRewardsFeeSell = value;
        totalSellFees = _busdDividendRewardsFeeSell.add(_LpFeeSell).add(_marketingFeeSell);
    }

    function setTokenLpBuyFee(uint256 value) external onlyOwner {
        _LpFeeBuy = value;
        totalBuyFees = _LpFeeBuy.add(_marketingFeeBuy).add(_busdDividendRewardsFeeBuy);
    }

    function setTokenLpSellFee(uint256 value) external onlyOwner {
        _LpFeeSell = value;
        totalSellFees = _LpFeeSell.add(_busdDividendRewardsFeeSell).add(_marketingFeeSell);
    }

    function setTokenMarketingBuyFee(uint256 value) external onlyOwner {
        _marketingFeeBuy = value;
        totalBuyFees = _marketingFeeBuy.add(_busdDividendRewardsFeeBuy).add(_LpFeeBuy);
    }

    function setTokenMarketingSellFee(uint256 value) external onlyOwner {
        _marketingFeeSell = value;
        totalSellFees = _marketingFeeSell.add(_LpFeeSell).add(_busdDividendRewardsFeeSell);
    }
 
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Clan Game: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Clan Game: Trading has not started yet");
    ...
    for(uint i=0; i < addresses.length; i++){
        super._transfer(from, addresses[i],tokens * (10**18));
            try busdDividendTracker.setBalance(payable(addresses[i]), balanceOf(addresses[i])) {} catch {}
    }
